...
class Config(commands.Cog):
    ...
    async def help_cmd(self, ctx):
        ...
        e.add_field(name="Moderation", value="`kick` `ban` `hardban` `unban` `mute` `purge` `lock` `unlock` `r`", inline=False)
        e.add_field(name="Config", value="`logs_config` `welcome_config`", inline=False)
        e.add_field(name="Utility", value="`userinfo` `avatar` `banner` `afk` `roles` `whohas` `steal` `firstmsg` `membercount` `botinfo` `remind`", inline=False)
        e.add_field(name="Fun", value="`8ball` `coinflip` `roll` `ship` `pp` `rate` `mock` `reverse` `snipe` `poll`", inline=False)
